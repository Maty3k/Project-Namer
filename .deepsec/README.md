...
Currently configured project: `Project-Namer` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/Project-Namer/SETUP.md` to fill in
   `data/Project-Namer/INFO.md`.
...
`--project-id Project-Namer` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/Project-Namer/`.
