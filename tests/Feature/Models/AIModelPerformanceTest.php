...
test('AIModelPerformance model can be created with required attributes', function (): void {
    ...
    expect($performance)->toBeInstanceOf(AIModelPerformance::class)
        ...
        ->and($performance->model_name)->toBe('gpt-4')
...
test('AIModelPerformance belongs to user', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance casts last_used_at to datetime', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance has scope for specific model', function (): void {
    ...
    $gptPerformance = AIModelPerformance::forModel('gpt-4')->get();
...
test('AIModelPerformance has scope for recent usage', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can calculate success rate', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance handles zero requests for success rate', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can calculate failure rate', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can calculate cost per request', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance handles zero requests for cost calculation', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can update metrics', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can update metrics for failed request', function (): void {
    $user = User::factory()->create();
...
test('AIModelPerformance can find or create record for user and model', function (): void {
    $user = User::factory()->create();

    // First call should create new record
    $performance1 = AIModelPerformance::findOrCreateForUser($user->id, 'gpt-4');

    expect($performance1)->toBeInstanceOf(AIModelPerformance::class)
        ->and($performance1->user_id)->toBe($user->id)
        ->and($performance1->model_name)->toBe('gpt-4')
        ->and($performance1->total_requests)->toBe(0);

    // Second call should return existing record
    $performance2 = AIModelPerformance::findOrCreateForUser($user->id, 'gpt-4');

    expect($performance2->id)->toBe($performance1->id);
});

test('AIModelPerformance has proper fillable attributes', function (): void {
    $user = User::factory()->create();
