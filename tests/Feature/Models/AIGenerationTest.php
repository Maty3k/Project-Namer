...
test('AIGeneration model can be created with required attributes', function (): void {
    ...
    expect($aiGeneration)->toBeInstanceOf(AIGeneration::class)
        ...
        ->and($aiGeneration->models_requested)->toBe(['gpt-4', 'claude-3.5-sonnet'])
...
test('AIGeneration belongs to project', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration belongs to user', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration casts arrays and boolean properly', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration has scope for active generations', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration has scope for completed generations', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration has scope for recent generations', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration can check if generation is in progress', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration can mark as completed', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
    ...
    $metadata = ['total_time_ms' => 2500, 'model_used' => 'gpt-4'];
...
test('AIGeneration can mark as failed', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration can get duration in seconds', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
...
test('AIGeneration returns null duration when not started', function (): void {
    $user = User::factory()->create();
    $project = Project::factory()->create(['user_id' => $user->id]);
