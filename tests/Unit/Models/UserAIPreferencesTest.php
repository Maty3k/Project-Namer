...
test('UserAIPreferences model can be created with required attributes', function () {
    ...
    expect($preferences)->toBeInstanceOf(UserAIPreferences::class)
        ...
        ->and($preferences->preferred_models)->toBe(['gpt-4o', 'claude-3.5-sonnet'])
...
test('UserAIPreferences belongs to user', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences casts arrays and boolean properly', function () {
    $user = User::factory()->create();
    
    $preferences = UserAIPreferences::create([
        'user_id' => $user->id,
        'preferred_models' => json_encode(['gpt-4o', 'claude-3.5-sonnet']),
        'default_generation_mode' => 'creative',
        'default_deep_thinking' => 1,
        'model_priorities' => json_encode(['gpt-4o' => 1]),
        'custom_parameters' => json_encode(['temperature' => 0.8]),
        'notification_settings' => json_encode(['email_on_completion' => true]),
    ]);
...
test('UserAIPreferences can find or create for user', function () {
    $user = User::factory()->create();
    ...
    expect($preferences1)->toBeInstanceOf(UserAIPreferences::class)
        ...
        ->and($preferences1->preferred_models)->toBe(['gpt-4o', 'claude-3.5-sonnet'])
...
test('UserAIPreferences can get preferred models in priority order', function () {
    $user = User::factory()->create();
    ...
    expect($ordered)->toBe(['claude-3.5-sonnet', 'gpt-4o', 'gemini-1.5-pro']);
});

test('UserAIPreferences returns preferred models as-is when no priorities set', function () {
    $user = User::factory()->create();
    ...
    expect($ordered)->toBe(['gpt-4o', 'claude-3.5-sonnet', 'gemini-1.5-pro']);
});

test('UserAIPreferences can update preferred models', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences can update model priorities', function () {
    $user = User::factory()->create();
    
    $preferences = UserAIPreferences::factory()->create([
        'user_id' => $user->id,
        'model_priorities' => ['gpt-4o' => 1, 'claude-3.5-sonnet' => 2],
    ]);
...
test('UserAIPreferences can update custom parameters', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences can update notification settings', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences can check if model is preferred', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences can get default generation parameters', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences can validate generation mode', function () {
    $user = User::factory()->create();
...
test('UserAIPreferences has scope for users with preferred model', function () {
...
test('UserAIPreferences has proper fillable attributes', function () {
    $user = User::factory()->create();
