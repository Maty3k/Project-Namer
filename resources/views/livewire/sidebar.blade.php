<div class="{{ $collapsed ? 'w-16' : 'w-64' }} transition-all duration-500 ease-out transform {{ $collapsed ? 'translate-x-0' : 'translate-x-0' }} border-r h-screen flex flex-col themed-sidebar will-change-transform bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-700">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-slate-600 transition-all duration-300 ease-out">
        <div class="flex items-center transition-all duration-300 ease-out {{ $collapsed ? 'justify-center' : 'justify-between' }}">
            <div class="overflow-hidden transition-all duration-500 ease-out {{ $collapsed ? 'max-w-0 opacity-0' : 'max-w-full opacity-100' }}">
                @if(!$collapsed)
                    <h2 class="text-lg text-gray-900 dark:text-white font-semibold whitespace-nowrap transition-opacity duration-300 ease-out">Projects</h2>
                @endif
            </div>
            <flux:button
            ...
            >
                <div class="transition-transform duration-300 ease-out {{ $collapsed ? 'rotate-0' : 'rotate-180' }}">
                    @if($collapsed)
                        <x-app-icon name="expand" size="md" class="transition-colors duration-200" />
                    @else
                        <x-app-icon name="collapse" size="md" class="transition-colors duration-200" />
                    @endif
                </div>
            </flux:button>
        </div>
        ...
        <div class="overflow-hidden transition-all duration-500 ease-out {{ $collapsed ? 'max-h-0 opacity-0 mt-0' : ($this->projectCount > 0 ? 'max-h-20 opacity-100 mt-1' : 'max-h-0 opacity-0 mt-0') }}">
            @if(!$collapsed && $this->projectCount > 0)
                <p class="text-sm text-gray-500 dark:text-gray-400 transition-opacity duration-300 ease-out delay-150">{{ $this->projectCount }} projects</p>
            @endif
        </div>
    </div>
            ...
            <div class="overflow-hidden transition-all duration-500 ease-out {{ $collapsed ? 'max-h-0 opacity-0' : 'max-h-full opacity-100' }}">
                @if(!$collapsed)
                    <div class="p-4 text-center transition-all duration-300 ease-out delay-200">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-1 transition-opacity duration-300 ease-out">No projects yet</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 transition-opacity duration-300 ease-out delay-75">Create your first project</p>
                    </div>
                @endif
            </div>
        @else
                        ...
                        @if($collapsed)
                            ...
                            <div class="flex items-center justify-center relative">
                                @if($project->selectedName)
                                    ...
                                    <svg class="w-6 h-6 {{ $this->isActiveProject($project) ? 'text-blue-500 dark:text-blue-400' : 'text-green-600 dark:text-green-400' }}"
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                @else
                                    <!-- Regular project icon -->
                                    <svg class="w-6 h-6 {{ $this->isActiveProject($project) ? 'text-blue-500 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400' }}"
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if(!$collapsed)
                                    <div class="flex items-start gap-3 group-hover:pr-8 transition-all duration-200">
                                        ...
                                        <div class="flex-1 min-w-0 transition-all duration-300 ease-out delay-150">
                                            <h3 class="text-sm text-gray-900 dark:text-white font-medium truncate transition-opacity duration-300 ease-out">
                                            ...
                                            @if($project->selectedName)
                                                ...
                                                <p class="text-xs text-gray-400 dark:text-gray-500 mt-1 truncate transition-opacity duration-300 ease-out delay-250">
                                                    {{ $this->truncateName($project->description, 25) }}
                                                </p>
                                            @else
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate transition-opacity duration-300 ease-out delay-200">
    ...
    <div class="overflow-hidden transition-all duration-500 ease-out {{ $collapsed ? 'max-h-0 opacity-0' : 'max-h-20 opacity-100' }}">
        @if(!$collapsed)
            <div class="p-4 border-t border-gray-200 dark:border-slate-600 transition-all duration-300 ease-out">
                <div class="text-xs text-gray-400 dark:text-gray-500 text-center transition-opacity duration-300 ease-out delay-200">
                    Project Workflow UI
                </div>
            </div>
        @endif
